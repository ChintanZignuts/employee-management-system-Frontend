...
console.log(resolveJobStatus(jobList.value.expiry_date).text)

function resolveEmploymentType(employmentType) {
...
const deleteItemConfirm = async () => {
  try {
    ...
    await axios.post(`job/delete/${deleteItemId.value}`, { permanent: permentDelete.value }, config)
...
const addNewJob = async jobData => {
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      const response = await axios.post(`job/update/${editJobData.value.id}`, jobData, config)
    ...
    <div v-else>
      ...
      >
        ...
        <template #item.posted="{ item }">
          <span>{{ getTimeAgo( item.raw.posted_date) }}</span>
        </template>
        ...
        <template #item.name="{ item }">
          <VChip pill>
            <VAvatar
              start
              :image="item.raw.company.logo_url?`http://127.0.0.1:8000/storage/logos/${item.raw.company.logo_url}`:null"
            />
